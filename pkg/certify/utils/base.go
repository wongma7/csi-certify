...
type TestSuite interface {
	...
	defineTests(testsuites.TestDriver, testpatterns.TestPattern)
}
...
func DefineTestSuite(driver testsuites.TestDriver, tsInits []func() TestSuite) {
...
// individual tests.
func skipUnsupportedTest(driver testsuites.TestDriver, pattern testpatterns.TestPattern) {
	...
	if len(pattern.SnapshotType) > 0 {
		...
		case testpatterns.DynamicCreatedSnapshot:
			_, isSupported = driver.(testsuites.SnapshottableTestDriver)
		default:
			isSupported = false
		}
		if !isSupported {
		...
		case testpatterns.InlineVolume:
			_, isSupported = driver.(testsuites.InlineVolumeTestDriver)
		case testpatterns.PreprovisionedPV:
			_, isSupported = driver.(testsuites.PreprovisionedPVTestDriver)
		case testpatterns.DynamicPV:
			_, isSupported = driver.(testsuites.DynamicPVTestDriver)
		default:
			isSupported = false
		}
...
type genericVolumeTestResource struct {
	...
	volume testsuites.TestVolume
}

var _ TestResource = &genericVolumeTestResource{}

func createGenericVolumeTestResource(driver testsuites.TestDriver, config *testsuites.PerTestConfig, pattern testpatterns.TestPattern) *genericVolumeTestResource {
	...
	r.volume = testsuites.CreateVolume(driver, config, volType)

	switch volType {
	case testpatterns.InlineVolume:
		framework.Logf("Creating resource for inline volume")
		if iDriver, ok := driver.(testsuites.InlineVolumeTestDriver); ok {
	...
	case testpatterns.PreprovisionedPV:
		...
		if pDriver, ok := driver.(testsuites.PreprovisionedPVTestDriver); ok {
...
func convertTestConfig(in *testsuites.PerTestConfig) framework.VolumeTestConfig {
